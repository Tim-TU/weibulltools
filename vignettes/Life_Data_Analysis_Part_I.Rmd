...
In the latter case we will use the function `estimate_cdf()` with `methods = "mr"`. To deal with survived and failed units we will use `estimate_cdf()` with `methods = "johnson"`. In order to use `estimate_cdf` we must convert the `shock` dataset to an appropriate format using the function `failure_data()`.  

```{r failure probabilities, results = 'asis'}
# Extract failure data from shock
failure_tbl <- failure_data(shock, x = Distance, status = Censor, id = id)
failure_tbl
