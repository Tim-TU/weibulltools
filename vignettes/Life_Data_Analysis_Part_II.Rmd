...
shock_failure_tbl <- reliability_data(shock, x = Distance, status = Censor, id = id)

# Rank Regression: 
# for rank_regression(), estimated failure probabilities are required: 
shock_cdf <- estimate_cdf(shock_failure_tbl, methods = "johnson")
...
library(purrr)
mrr_list <- map(distributions, function(distribution) {
  rank_regression(shock_cdf, distribution = distribution)
})
...
weibull_plot <- weibull_grid %>% 
  plot_mod(x = shock_cdf$characteristic, loc_sc_params = mrr_weibull$loc_sc_params,
                             distribution = "weibull",
...
ml_list <- map(distributions, function(distribution) {
  ml_estimation(x = shock$Distance, event = shock$Censor, distribution = distribution)
})
...
ml_prob <- predict_prob(q = seq(6200, 30600, length.out = 100), 
                        loc_sc_params = ml_weibull$loc_sc_params, 
                        distribution = "weibull")

weibull_both <- weibull_plot %>% 
  plotly::add_lines(x = seq(6200, 30600, length.out = 100), y = SPREDA::qsev(ml_prob), 
...
lognormal_plot <- lognormal_grid %>% 
  plot_mod(x = df_alloy$characteristic, loc_sc_params = ml_lognormal3$loc_sc_params,
                             distribution = "lognormal3",
...
ml_prob_lognormal <- predict_prob(q = seq(85, 325, length.out = 100),
                        loc_sc_params = ml_lognormal$loc_sc_params,
                        distribution = "lognormal")

lognormal_both <- lognormal_plot %>%
  plotly::add_lines(x = seq(85, 325, length.out = 100), y = qnorm(ml_prob_lognormal),
            name = "Two-parametric Log-normal", color = I("#006400"), hoverinfo = "text",
            text = ~paste(paste("\u03BC<sub>ML</sub>", ":",
                                round(ml_lognormal$loc_sc_params[[1]], digits = 2)),
                          "<br>", paste("\u03C3<sub>ML</sub>", ":",
                                        round(ml_lognormal$loc_sc_params[[2]], digits = 2))))
lognormal_both
```
