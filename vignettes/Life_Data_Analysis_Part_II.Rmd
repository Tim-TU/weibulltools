...
shock_reliability_tbl <- reliability_data(shock, x = Distance, status = Censor, id = id)

# Rank Regression: 
# for rank_regression(), estimated failure probabilities are required: 
shock_cdf <- estimate_cdf(shock_reliability_tbl, methods = "johnson")
...
ml_list <- map(distributions, function(distribution) {
  ml_estimation(shock_reliability_tbl, distribution = distribution)
})
...
cycles_reliability_tbl <- reliability_data(x = cycles, status = state, id = id)

# Two-parameter Log-normal:  
ml_lognormal <- ml_estimation(
  cycles_reliability_tbl,
...
johnson_cdf_tbl <- estimate_cdf(cycles_reliability_tbl, "johnson")
