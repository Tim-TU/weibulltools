...
ml_list <- map(distributions, function(distribution) {
  ml_estimation(x = shock$Distance, status = shock$Censor, distribution = distribution)
})
...
lognormal_grid <- plot_prob(
  x = df_alloy$characteristic, y = df_alloy$prob, 
...
lognormal_plot <- lognormal_grid %>% 
  plot_mod(
    x = df_alloy$characteristic, 
    loc_sc_params = ml_lognormal3$loc_sc_params,
    distribution = "lognormal3",
