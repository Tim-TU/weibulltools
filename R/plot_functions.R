...
plot_layout <- function(x, distribution = "weibull",
...
plot_prob <- function(x, y, event, id = rep("XXXXXX", length(x)),
  ...
  p %>%
    plotly::add_trace(
      x = ~x_s, y = ~SPREDA::qsev(y_s), type = "scatter",
...
#' Add estimated Regression line to Probability Plot
#'
#' This function is used to add an estimated regression line to an existing
#' probability plot.
#'
...
#'   The default value of \code{y} is \code{NULL}. If \code{y} is set \code{NULL}
#'   the y-coordinates with respect to \code{x} are calculated by
#'   \code{loc_sc_params}.
...
plot_mod <- function(p_obj, x, y = NULL, loc_sc_params,
  ...
  p_mod <- plotly::add_lines(
    p = p_obj, data = df_p, x = ~x_p, y = ~SPREDA::qsev(y_p),
    type = "scatter", mode = "lines", hoverinfo = "text", color = I("#CC2222"),
    name = title_trace,
...
plot_conf <- function(p_obj, x, y, direction = c("y", "x"),
  ...
  p_conf <- plotly::add_lines(
    ...
    x = ~x, y = ~SPREDA::qsev(y), type = "scatter", mode = "lines",
...
#' @param x a numeric vector containing the x-coordinates of the regression line.
#' @param params a (named) numeric vector, where the first entry is the scale
#'   parameter and the second entry is the shape parameter of a weibull
#'   distribution. Parametrization is the same used in \code{\link{rweibull}}.
#' @param distribution supposed distribution of the random variable. The default
#'   value is \code{"weibull"}.
#' @param title_trace a character string whis is assigned to the trace shown in
#'   the legend.
...
#'
plot_pop <- function(p_obj, x, params, distribution = "weibull",
  ...
  p_pop <- plotly::add_lines(p = p_obj, x = ~x_s, y = ~SPREDA::qsev(y_s),
...
plot_prob2 <- function(x, y, event, mrr_output = NULL, id = rep("XXXXXX", length(x)),
                       distribution = "weibull",
                       title_main = "Probability Plot",
                       title_x = "Characteristic",
                       title_y = "Unreliability",
  ...
  if(!is.null(mrr_output)) {
    ...
    plot <- p %>% plotly::add_trace(x = ~x_s, y = ~SPREDA::qsev(y_s), type = "scatter",
...
#' @param distribution supposed distribution of the random variable. The default value is \code{"weibull"}.
...
plot_mod2 <- function(p_obj, x, y = NULL, mrr_output,
                      distribution = "weibull",
                      title_trace = "Fit") {
  ...
  if(exists("mod_2", where = mrr_output)) {
    ...
    plot <- plot <- p_obj %>% plot_mod_groups(x = x,loc_sc_params = mrr_output$loc_sc_coefficients, color = "blue")
  }
  return(plot)
}
